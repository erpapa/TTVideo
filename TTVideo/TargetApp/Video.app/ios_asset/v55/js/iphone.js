...
((__t=(article.publishTime))==null?'':_.escape(__t))+
'</div></div><div class="bottom-buttons only-two"><div id="digg" data-answerid="'+
...
((__t=( wenda.aniok ))==null?'':_.escape(__t))+
'"><span><i>&nbsp;</i><span class="b bury-count" realnum="0">踩</span></span></div></div>';
 } 
__p+='';
}
return __p;
	...
	if (globalArticleObject.article.type === 'forum') {
		if ($('.poi').length > 0) {
			$('.poi').append('<span class="bottomtime">创建于' + globalArticleObject.article.publishTime + '</span>');
		} else {
...
function set_info (states) {
    ...
    if ('is_digg' in states && 'digg_count' in states) {
        $('#digg').attr('wenda-state', states.is_digg ? 'digged' : '');
    ...
    if ('is_buryed' in states && 'bury_count' in states) {
        $('#bury').attr('wenda-state', states.is_buryed ? 'buryed' : '');
        formatCount('.bury-count', states.bury_count, '踩');
    }

    if (is_app_version_bigger_than('5.6')) {
	...
	function showImage (type) {
	    ...
	    if( holders_len ){
		    ...
		    if(type !== 'origin'){
		    	...
		    	toggle_node.innerHTML = '<a class="toggle-img tt-font-2 tt-face-4 tt-thin-border" id="toggle-img" href="javascript:;" tt-press>显示大图</a>';
